...
```sh
# The workspace must be mounted at the SAME path inside and outside the container:
# brigade passes workspace paths to the host Docker daemon for session bind-mounts.
...
Docker session mode also needs the agent image on the host:
