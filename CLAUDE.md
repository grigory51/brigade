...
- **ConnectRPC** — unary-вызовы `AuthService`/`SessionService`/`AgentService`. Авторизация
  через `auth.Interceptor` (кладёт пользователя в контекст из Bearer или cookie).
...
- **AG-UI поверх SSE** — `POST /api/ag-ui/{run,permission}` (ACP-режим). Аутентификация
  **Bearer access-JWT на каждый запрос** (не тикет); `threadId` = идентификатор сессии.
- **Встроенный SPA** — `mux.Handle("/")`, фронт через `go:embed` из
  `backend/internal/web/dist`.

> Важно: ACP-режим идёт по **SSE**, не по WebSocket. (Историческая заметка в коде может
> упоминать WS — актуально SSE.)
