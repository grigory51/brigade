...
type Spec struct {
	...
	// ClaudeHomeHost — путь на хосте к персональному ~/.claude пользователя
	// (<claude_home_dir>/<userID>), bind-mount'ится в /home/agent/.claude контейнера.
	// Общий для всех контейнеров пользователя: авторизация Claude (`/login`) делается
	// один раз и видна во всех его сессиях. Пусто — не монтировать (фича выключена).
